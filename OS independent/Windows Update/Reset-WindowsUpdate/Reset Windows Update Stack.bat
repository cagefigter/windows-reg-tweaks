...
net stop bits
echo Checking the current bits service status.
...
cls
echo.
echo Cannot reset Windows Update because "Background Intelligent Transfer Service" (bits) service failed to stop.
echo.
pause
goto Start
...
cls
echo.
echo Cannot reset Windows Update Stack since "Application Identity" (appidsvc) service failed to stop.
echo.
pause
goto Start
...
cls
echo.
echo Cannot reset Windows Update Stack since "Cryptographic Services" (cryptsvc) service failed to stop.
echo.
pause
goto Start
...
REM Renaming AU cache folder and log file...
...
:: Re-register the BITS files and the Windows Update files
